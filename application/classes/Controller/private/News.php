...
class Controller_Private_News extends Controller_Base_Admin {
    /*
     * 1.list
     * 2.add
     * 3.do_add
    ...
    /*
     * Geting all news list
     */

    public function action_list() {
        $session = Session::instance();
        $lng = $session->get('lng');
        $list = Model::factory('Private_News')->getNews($lng);
        if ($list == null)
            $list = array();
        else {
            for ($i = 0; $i < count($list); $i++) {
                $list[$i]['date_of_creation'] = Libs__Util::dateFormat($lng, $list[$i]['date_of_creation']);
            }
        }
        $this->template->page = View::factory('private/pages/news/list')
                ->bind('list', $list);
    }
    ...
    public function action_add() {
        ...
        $this->template->page = View::factory('private/pages/news/add')
                ->bind('lang', $lang);
    }
    ...
    public function action_do_add() {
        ...
        if ($is_ok) {
            ...
            ############ END  SEO  block  ########################
            $result = Model::factory('Private_News')->add($page, $images);
